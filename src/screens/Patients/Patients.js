...
import { useGetPatientsCardsQuery, useGetPatientsQuery, useDeletePatientMutation } from '../../redux/services/patient';
...
function Patients() {
  ...
  const [deletePatient, { data: deleteData }] = useDeletePatientMutation({})

  const { data: patientData, isLoading: loadingPatientData, refetch: refetchPatientsCard } = useGetPatientsCardsQuery({})

  const { data, isLoading, refetch: refetchPatients } = useGetPatientsQuery({})
  ...
  useEffect(() => {
    refetchPatients()
    refetchPatientsCard()
  }, [navigate, refetchPatients, refetchPatientsCard])
