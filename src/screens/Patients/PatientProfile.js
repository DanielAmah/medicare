...
import { useGetPatientQuery } from '../../redux/services/patient'

function PatientProfile() {
  const { id } = useParams();
  console.log(id, 'id')
  const { data } = useGetPatientQuery({ id: id })
  ...
  const tabPanel = () => {
    switch (activeTab) {
      ...
      case 2:
        return <AppointmentsUsed doctor={false} appointments={data?.appointments} />;
      case 3:
        return <InvoiceUsed  invoices={data?.invoices}/>;
      case 4:
        return <PaymentsUsed doctor={false} payments={data?.payments} />;
      case 5:
  ...
  return (
    <Layout>
      ...
      <div className=" grid grid-cols-12 gap-6 my-8 items-start">
        ...
        >
          <img
            src={data?.image || "/images/user7.png"}
            alt="setting"
