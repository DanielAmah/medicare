...
function DoctorProfile() {
  ...
  const { data } = useGetDoctorQuery({ id: id })
  ...
  const tabPanel = () => {
    switch (activeTab) {
      ...
      case 2:
        return <PatientsUsed patients={data?.patients} />;
      case 3:
        return <AppointmentsUsed doctor={true} appointments={data?.appointments} />;
      case 4:
        return <PaymentsUsed doctor={true} payments={data?.payments} />;
      case 5:
        return <InvoiceUsed invoices={data?.invoices} />;
      case 6:
  ...
  return (
    <Layout>
      ...
      <div className=" grid grid-cols-12 gap-6 my-8 items-start">
        ...
        >
          <img
            src={data?.profile_image_url || "/images/user1.png"}
            alt="setting"
            className="w-40 h-40 rounded-full object-cover border border-dashed border-subMain"
          />
          <div className="gap-2 flex-colo">
            <h2 className="text-sm font-semibold">Dr. {data?.name}</h2>
