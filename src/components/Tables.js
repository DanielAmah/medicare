      ...
      <thead className="bg-dry rounded-md overflow-hidden">
        <tr>
          ...
          <th className={thclass}>
            Amount <span className="text-xs font-light">($)</span>
          </th>
          ...
          >
            ...
            <td className={tdclass}>
              <div className="flex gap-4 items-center">
                <span className="w-12">
                  <img
                    src={item.user.image || `/images/user${item.user.id}.png`}
                    alt={item.user.title}
          ...
          >
            ...
            <td className={tdclass}>
              <div className="flex gap-4 items-center">
                <span className="w-12">
                  <img
                    src={item?.to?.image || 'images/user1.png'}
                    alt={item?.to?.title}
                    className="w-full h-12 rounded-full object-cover border border-border"
                  />
                </span>
      ...
      <tbody>
        {data?.map((item, index) => (
          ...
          >
            ...
            <td className={tdclasse}>
              <div className="flex gap-4 items-center">
                {!used && (
                  <span className="w-12">
                    <img
                      src={item.image || `/images/user${item.id}.png`}
                      alt={item.title}
                      className="w-full h-12 rounded-full object-cover border border-border"
                    />
                  </span>
            ...
            <td className={tdclasse}>
              <span
                className={`py-1 px-4 ${item.gender || 'Female' === 'Male'
                  ? 'bg-subMain text-subMain'
                  : 'bg-orange-500 text-orange-500'
                  } bg-opacity-10 text-xs rounded-xl`}
              >
            ...
            {!used && (
              <>
                <td className={tdclasse}>{item.blood || 'A+'}</td>
                <td className={tdclasse}>{item.age || (32 + index)}</td>
              </>
            )}

            <td className={tdclasse}>
              <MenuSelect datas={DropDown1(item.id)} item={item}>
                <div className="bg-dry border text-main text-xl py-2 px-4 rounded-lg">
                  <BiDotsHorizontalRounded />
                </div>
              </MenuSelect>
          ...
          >
            ...
            <td className={tdclass}>
              <div className="flex gap-4 items-center">
                <span className="w-12">
                  <img
                    src={item.user.image || '/images/user4.png'}
                    alt={item.user.title}
                    className="w-full h-12 rounded-full object-cover border border-border"
                  />
                </span>
        ...
        {data?.length ? data?.map((item) => (
          ...
          >
            ...
            <td className={tdclass} style={{ textAlign: 'center' }}>
              <p className="text-xs" style={{ textAlign: 'center' }}>No invoices</p>
            </td>
            <td className={tdclass}>
              <p className="text-xs"></p>
            </td>
